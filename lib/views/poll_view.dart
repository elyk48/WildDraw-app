                                                          ...
                                                          child: Card(
                                                            child: Padding(
                                                              ...
                                                              child: LinearPercentIndicator(
                                                                ...
                                                                progressColor:Color(0xFF911A00),
                                                              ),
                                                            ),
                                                              ...
                                                              /*else
                                                   {
                                                     ...
                                                     await FirebaseFirestore.instance.collection('poll').doc(snapshot.data.docs[i].id).collection('users').doc(user.id).set({
                                                       "vote": snapshot2.data.docs[j].id,
                                                            ...
                                                            // print( GetAllVotes(snapshot.data.docs[i]["question"]).toString()+"hethi");
                                                            print("Option Chosen: " +
                                                                snapshot2.data.docs[j]["name"]);
                                                          }
                                                        },
...
/*int GetAllVotes(String id){
 int s =0;
 int i= 0;
    FirebaseFirestore.instance.collection("poll").doc(id).collection("options").get().then((value) {
 //print(value.docs.length.toString());
      if(value.docs.isNotEmpty){
      for( i= 0;i<value.docs.length;i++){

        s+=int.parse(value.docs[i].data()['votes']);



      }
print(s.toString()+"yeeeee");
    return s;
    }
    }
      );
